...
async function adapter(params) {
    ...
    const normalizeUrl =  true ? new URL(params.request.url) : 0;
...
const users = [
    {
        id: "1",
        nfcUid: "04:A2:E9:B2:C1:D3",
        accessCode: "ACCESS-X1G4",
        isAuthenticated: false,
        glyphName: "⌂-CR0SS-⌂"
    },
    {
        id: "2",
        nfcUid: "7A:9F:E1:D3:B5:C2",
        accessCode: "ACCESS-Z9F2",
        isAuthenticated: false,
        glyphName: "◊-VOID-◊"
    },
    {
        id: "3",
        nfcUid: "B1:C3:D5:E7:F9:A2",
        accessCode: "ACCESS-T8K3",
        isAuthenticated: false,
        glyphName: "Ω-DAWN-Ω"
    }
];
...
const db = {
    // User authentication
    findUserByNfcUid: (nfcUid)=>{
        return users.find((user)=>user.nfcUid === nfcUid);
    },
    validateAccessCode: (nfcUid, accessCode)=>{
        const user = users.find((user)=>user.nfcUid === nfcUid && user.accessCode === accessCode);
        return user;
    },
    setUserAuthenticated: (userId, token)=>{
    ...
    verifySession: (userId, token)=>{
        ...
        const isValid = user.isAuthenticated && user.sessionToken === token && user.sessionExpiry !== undefined && user.sessionExpiry > new Date();
        return isValid;
    },
    logoutUser: (userId)=>{
        const userIndex = users.findIndex((user)=>user.id === userId);
    ...
    getMessagesForRoom: (roomId)=>{
        ...
        return chatMessages.filter((msg)=>msg.roomId === roomId).sort((a, b)=>a.timestamp.getTime() - b.timestamp.getTime());
    },
    addMessage: (roomId, sender, message)=>{
        const newMessage = {
            id: esm_browser_v4(),
            roomId,
            sender,
        ...
        if (roomMessages.length > 100) {
            ...
            const oldestMessages = roomMessages.sort((a, b)=>a.timestamp.getTime() - b.timestamp.getTime()).slice(0, messagesToRemove).map((msg)=>msg.id);
...
const generateGlyphName = (userId)=>{
...
const authService = {
    ...
    authenticate: async (nfcUid, accessCode)=>{
        try {
            ...
            const token = await new SignJWT({
                ...
                alg: "HS256"
            }).setIssuedAt().setExpirationTime(TOKEN_EXPIRY).setJti(esm_browser_v4()).sign(JWT_SECRET);
    ...
    verifyToken: async (token)=>{
        try {
            ...
            return {
                ...
                nfcUid: payload.nfcUid,
                glyphName: payload.glyphName,
                exp: payload.exp
            };
        } catch (error) {
...
;// CONCATENATED MODULE: ./node_modules/next/dist/build/webpack/loaders/next-middleware-loader.js?absolutePagePath=private-next-root-dir%2Fsrc%2Fmiddleware.ts&page=%2Fsrc%2Fmiddleware&rootDir=%2FUsers%2Flucastravers%2FDesktop%2FENO-YEK&matchers=W3sicmVnZXhwIjoiXig%2FOlxcLyhfbmV4dFxcL2RhdGFcXC9bXi9dezEsfSkpP1xcL2NoYXQoPzpcXC8oKD86W15cXC8jXFw%2FXSs%2FKSg%2FOlxcLyg%2FOlteXFwvI1xcP10rPykpKikpPyguanNvbik%2FW1xcLyNcXD9dPyQiLCJvcmlnaW5hbFNvdXJjZSI6Ii9jaGF0LzpwYXRoKiJ9LHsicmVnZXhwIjoiXig%2FOlxcLyhfbmV4dFxcL2RhdGFcXC9bXi9dezEsfSkpP1xcL2FwaVxcL2NoYXQoPzpcXC8oKD86W15cXC8jXFw%2FXSs%2FKSg%2FOlxcLyg%2FOlteXFwvI1xcP10rPykpKikpPyguanNvbik%2FW1xcLyNcXD9dPyQiLCJvcmlnaW5hbFNvdXJjZSI6Ii9hcGkvY2hhdC86cGF0aCoifV0%3D&preferredRegion=&middlewareConfig=eyJtYXRjaGVycyI6W3sicmVnZXhwIjoiXig%2FOlxcLyhfbmV4dFxcL2RhdGFcXC9bXi9dezEsfSkpP1xcL2NoYXQoPzpcXC8oKD86W15cXC8jXFw%2FXSs%2FKSg%2FOlxcLyg%2FOlteXFwvI1xcP10rPykpKikpPyguanNvbik%2FW1xcLyNcXD9dPyQiLCJvcmlnaW5hbFNvdXJjZSI6Ii9jaGF0LzpwYXRoKiJ9LHsicmVnZXhwIjoiXig%2FOlxcLyhfbmV4dFxcL2RhdGFcXC9bXi9dezEsfSkpP1xcL2FwaVxcL2NoYXQoPzpcXC8oKD86W15cXC8jXFw%2FXSs%2FKSg%2FOlxcLyg%2FOlteXFwvI1xcP10rPykpKikpPyguanNvbik%2FW1xcLyNcXD9dPyQiLCJvcmlnaW5hbFNvdXJjZSI6Ii9hcGkvY2hhdC86cGF0aCoifV19!
